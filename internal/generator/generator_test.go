...
func TestBuildFilteredDescriptor_CrossFileMessage(t *testing.T) {
	...
	msgDesc := &descriptorpb.DescriptorProto{
		...
		Field: []*descriptorpb.FieldDescriptorProto{
			{Name: proto.String("id"), Number: proto.Int32(1), Type: descriptorpb.FieldDescriptorProto_TYPE_STRING.Enum()},
			{Name: proto.String("name"), Number: proto.Int32(2), Type: descriptorpb.FieldDescriptorProto_TYPE_STRING.Enum(), Options: makeFieldOptions(OperationCreate)},
			{Name: proto.String("email"), Number: proto.Int32(3), Type: descriptorpb.FieldDescriptorProto_TYPE_STRING.Enum(), Options: makeFieldOptions(OperationCreate)},
		},
	}
...
func TestBuildFilteredDescriptor_EmptySyntheticForNoMatchingOps(t *testing.T) {
	// Message has field_op annotations but none match DELETE.
	// The synthetic should be created with zero fields.
	fdp := &descriptorpb.FileDescriptorProto{
		Name:    proto.String("test.proto"),
		Package: proto.String("test.v1"),
		MessageType: []*descriptorpb.DescriptorProto{
			{
				...
				Field: []*descriptorpb.FieldDescriptorProto{
					{Name: proto.String("id"), Number: proto.Int32(1), Type: descriptorpb.FieldDescriptorProto_TYPE_STRING.Enum(), Options: makeFieldOptions(OperationCreate, OperationRead)},
					{Name: proto.String("name"), Number: proto.Int32(2), Type: descriptorpb.FieldDescriptorProto_TYPE_STRING.Enum(), Options: makeFieldOptions(OperationCreate, OperationRead)},
				},
			},
		},
	...
	allMessages := map[string]*descriptorpb.DescriptorProto{
		"test.v1.Item": fdp.GetMessageType()[0],
	}
	allTypeFiles := map[string]string{
		"test.v1.Item": "test.proto",
	}
	...
	if synthetic == nil {
		t.Fatal("synthetic Item_DELETE not found — should be created even with no matching fields")
	}
...
func TestGetFieldsForOpRaw(t *testing.T) {
	msg := &descriptorpb.DescriptorProto{
		Name: proto.String("TestMsg"),
		Field: []*descriptorpb.FieldDescriptorProto{
			{Name: proto.String("a"), Number: proto.Int32(1), Options: makeFieldOptions(OperationCreate, OperationUpdate)},
			{Name: proto.String("b"), Number: proto.Int32(2), Options: makeFieldOptions(OperationRead)},
			{Name: proto.String("c"), Number: proto.Int32(3), Options: makeFieldOptions(OperationCreate, OperationRead, OperationUpdate)},
			{Name: proto.String("d"), Number: proto.Int32(4)}, // no annotation
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.op, func(t *testing.T) {
			got := getFieldsForOpRaw(msg, tt.op)
			if len(got) != len(tt.wantNames) {
				t.Fatalf("getFieldsForOpRaw(%s) returned %d fields, want %d", tt.op, len(got), len(tt.wantNames))
			}
			for i, fi := range got {
				if fi.Name != tt.wantNames[i] {
					t.Errorf("field[%d].Name = %q, want %q", i, fi.Name, tt.wantNames[i])
				}
			}
		})
	}
